...
- **📈 Aggregation & Dashboards**
  ...
  - Student dashboard: enrollments, completed courses, average score, per-course progress
  - Uses MongoDB Aggregation Pipelines (`$lookup`, `$group`, `$avg`, `$cond`, `$round`)

- **🛡️ Data Validation & Error Handling**
  - Request body validation using **Joi**
  - Centralized global error handling via custom `AppError` class
...
Login to receive token and refreshToken:
...
Body: { "refreshToken": "..." }
Promoting a user to Instructor or Admin must be done directly in MongoDB (e.g. via Compass):
...
Add environment variables (MONGO_URI, JWT_SECRET, JWT_REFRESH_SECRET, etc.).

Deploy 🚀

Update the Swagger servers.url in config/swagger.js to your live URL.
